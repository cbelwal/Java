package com.dasmic.android.lib.audio.Extension;
...
public class ListViewAdapter extends
        ArrayAdapter<DataAudioDisplay> implements Filterable{
    ...
    private final ArrayList<DataAudioDisplay> _contactsData;
    private ArrayList<DataAudioDisplay> _filteredData;
    ...
    public ListViewAdapter(ActivityBaseMain activityBaseMain, ArrayList<DataAudioDisplay> contactsData) {
    ...
    public View getView(int position,View rowView, ViewGroup parent) {
        ...
        dad=_filteredData.get(position);

        CheckedTextView chTextView = holder.ctv;
        chTextView.setText(dad.getPrimaryValue());


        chTextView.setTag(dad);
        chTextView.setChecked(dad.IsChecked);
        //textView.setFocusable(false);
        chTextView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ((CheckedTextView) v).toggle();
                ((DataAudioDisplay) v.getTag()).IsChecked = ((CheckedTextView) v).isChecked();
                //Raise event
                _onCheckBoxClicked.onEvent(((DataAudioDisplay) v.getTag()).IsChecked ? 1 : 0);

            }
        });

        TextView tvSecondary = holder.tvSecondary;//(TextView) rowView.findViewById(R.id.secondaryTextView);
        tvSecondary.setText(dad.getSecondaryValue(_activityBaseMain.getCurrentDisplayOption()));
        tvSecondary.setTag(chTextView);
        tvSecondary.setOnClickListener(new View.OnClickListener() {
                                           @Override
                                           public void onClick(View v) { //Simulate click on checkedTextView
        ...
        tvTertiary.setText(dad.getTertiaryValue());
        ...
        iv.setOnClickListener(new View.OnClickListener() {
                                  ...
                                  public void onClick(View v) { //When Image is clicked do some action base on type
                                      DataAudioDisplay dvd =
                                              (DataAudioDisplay) v.getTag();
                                      _activityBaseMain.onClickItemImage(dvd);
                                  }
                              }
        );


        Drawable icon = getDrawableObject(dad);
        if (icon != null)
            iv.setImageDrawable(icon);

        LinearLayout ll = holder.ll;
        ll.setBackgroundColor(getColor(dad));
    ...
    private Drawable getDrawableObject(DataAudioDisplay dfd){
        //Is a Video file
        return getSpecialFileThumbnail(dfd);

    }

    private Drawable getSpecialFileThumbnail(DataAudioDisplay dad){
        return dad.getThumbnail(_activityBaseMain);
    }


    //Construct color based on data
    private int getColor(DataAudioDisplay dad){
        ...
        if(dad.Size > 1024 * 1024)
            red = red + 60;
        else if (dad.Size > 1024 * 50)
            red=red + 30;
        else if (dad.Size > 1024)
            blue=blue+80;
    ...
    @Override
    public DataAudioDisplay getItem(int position) {
        //if()
    ...
    public Filter getFilter() {
        ...
        Filter filter = new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence constraint) {
                ...
                final ArrayList<DataAudioDisplay> currentData = _contactsData;

                int count = currentData.size();
                final ArrayList<DataAudioDisplay> filteredData =
                        new ArrayList<DataAudioDisplay>();

                DataAudioDisplay filterableData ;
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    if (filterableData.toString().toLowerCase().contains(filterString) ||
                            filterableData.getSecondaryValue(_activityBaseMain.getCurrentDisplayOption()).toLowerCase().contains(filterString)) {
            ...
            protected void publishResults(CharSequence constraint, FilterResults results) {
                ...
                _filteredData = (ArrayList<DataAudioDisplay>) results.values;
